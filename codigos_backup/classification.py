...
def update_lr(epoch, lr):
    ...
    if epoch <= 100:
        decay_rate = np.log(1e-5 / 1e-3) / (70  - 1)
...
masses = ['gamma', 'electron', 'helium', 'proton', 'nitrogen', 'silicon', 'iron', 'nitrogen']
hadrons = ['proton', 'nitrogen', 'silicon', 'iron', 'helium']
...
for mass in masses:
    ...
    #print(sorted(os.listdir(feature_path)))
    file_list = [int(tel_number.split('_')[-1][:-4]) for tel_number in sorted(os.listdir(feature_path))] ##ordenação das listas
    print(sorted(os.listdir(feature_path)))
    features = [pd.read_csv(os.path.join(feature_path, csv_file)) for csv_file in sorted(os.listdir(feature_path))]
    print(sorted(zip(file_list, os.listdir(feature_path))))
...
trig_number = 4
dataset = dataset[dataset['trigs'] == trig_number].sample(frac = 1, random_state = 1)
#da = features_4_tel.sample(frac = frac, random_state = 1)


print(np.unique(dataset['mass']))
...
split_number = 15
ds = separate_dataframe(dataset, split_number)
...
for number in range(5):
    for split, dataset in enumerate(ds):
        ...
        train_labels = tf.keras.utils.to_categorical(encoded_labels_train, num_classes=3)

        encoded_labels_test = label_encoder.transform(test['mass'])
        test_labels = tf.keras.utils.to_categorical(encoded_labels_test, num_classes=3)
        ...
        model.compile(optimizer=Adam(1e-4), loss='categorical_crossentropy', metrics=['accuracy','mae'])
        ...
        with tf.device('/GPU:0'):
            history = model.fit(
                ...
                y=train_labels,
                epochs= 1,
                validation_data=([test_tel_1, test_tel_2, test_tel_3, test_tel_4], test_labels),
                verbose=True
            )
...
output_path = os.path.join(main_path, 'tf-keras', 'classification', 'gamma-electron-hadron-concat')
...
try:
    ...
    print("Diretório existente!")
hist.to_csv(os.path.join(output_path, f'history.csv'))

#Salvar os modelos e os pesos
model.save(os.path.join(output_path, f'regresion.h5'))
model.save_weights(os.path.join(output_path, f'regresion.weights.h5'))
...
for split, dataset in enumerate(ds):
    ...
    test_inputs= ([test_tel_1, test_tel_2, test_tel_3, test_tel_4], test_labels)

    y_pred, y_true = cm.confusion_matrix_concat(model, test_inputs)
...
nome_do_grafico = f"confusion_matrix.png"
output_path = os.path.join(output_path, f"{nome_do_grafico}")

cm.plot_only(y_pred_reg, y_true_reg, np.unique(dataset['mass']), output_path, False, False)