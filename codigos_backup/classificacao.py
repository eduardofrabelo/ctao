...
def separate_dataframe(dataframe, chunk_size):
    ...
    dataframe = dataframe.sample(frac = 1, random_state = 15)
...
masses = ['gamma', 'electron', 'proton', 'nitrogen', 'silicon', 'iron', 'nitrogen']
...
for mass in masses:
    ...
    frac = 1
    features_4_tel = features_4_tel.sample(frac = frac, random_state = 15)
...
ds = separate_dataframe(dataset, 25)
...
for split, dataset in enumerate(ds):
    ...
    y_pred, y_true = cm.confusion_matrix_concat(model, ([test_tel_1, test_tel_2, test_tel_3, test_tel_4], test_labels))
...
cm.plot_only(y_pred_reg, y_true_reg, ['gamma', 'electron', 'hadron'], output_path, False, False)